...
containers.forEach(container => {
  ...
  input.addEventListener('focus', () => {
    ...
    if(!input.value){
      tl.fromTo(line, 
        {attr: {d: start}}, 
        {attr: {d: end}, ease: 'Power2.easeOut', duration: 0.35});
...
// Reverting back if it's not focused
...
const promoCont = document.querySelector('.promo-container');
...
promoCont.addEventListener('click', () => {
  if(checkbox.checked) {
    ...
    tl2.to('.checkbox-label', {color: '#6319e8'}, '<')
  } else {
...
const tl3 = gsap.timeline({
  defautls: {duration: 0.75, ease: 'Power2.easeOut'},
})
